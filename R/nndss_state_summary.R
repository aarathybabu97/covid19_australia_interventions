...
# wa_examine <- check_wa_linelist%>%
#   filter(state=="WA")%>%
#   filter(date_confirmation>="2022-08-20"& date_confirmation<="2022-08-29")%>%
#   group_by(state, date_confirmation, test_type) %>%
#   summarise(cases = n())
# 
# wa_examine%>%
...
data <- reff_model_data(linelist_raw = linelist_new,
