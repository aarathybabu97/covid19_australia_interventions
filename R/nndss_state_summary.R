...
linelist <-
  load_linelist(use_vic = FALSE) #skip Vic since using summary data

# remove dubious SA onset dates
# need to look at this again at some point
linelist$date_onset[(linelist$state == "SA" &
                       linelist$date_onset >= as_date("2022-02-27"))] <- NA
...
# add queensland, tasmania & act data
# Run covidlive_linelist script for scraped tas data
# uncomment the act commonwealth bits when the below script for the first time
source("R/tas_qld_act_data.R")

#read in vic to override the missing vic col
vic.files <-
  list.files("~/not_synced/vic/",
             pattern = "count",
             full.names = TRUE)
vic.dates <- vic.files %>%
  basename() %>%
...
vic_state_count <-
  read_csv(vic.files) %>% rename("PCR_VIC" = "confirmed",
                                 "RAT_VIC" = "probable",
                                 "Date" = "date")
...
qld_state_count <-
  read_csv("~/not_synced/qld/linelist_commonwealth_QLD.csv") %>%
  mutate(Date = as.Date(Date, format = "%Y%m%d"))

linelist_VIC_QLD_preprocess <-
  full_join(vic_state_count, qld_state_count)
...
linelist_VIC_QLD_TAS <- bind_rows(linelist_VIC_QLD, tas_data)

#make into reff data format
linelist_VIC_QLD_TAS$import_status <- "local"
act_commonwealth$import_status <- "local"

saveRDS(linelist_VIC_QLD_TAS, "outputs/linelist_vic_qld_tas")
  ...
  bind_rows(regular_ll %>%
              filter(
                date_confirmation < "2022-01-06" &
                  state %in% c("QLD", "VIC", "ACT", "TAS")
              ))# bound with qld, act & vic before 2022-01-06
...
source("R/watermelon_plot_completion_prob.R")
