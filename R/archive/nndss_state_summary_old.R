...
regular_ll%>%
  filter(date_confirmation==max(regular_ll$date_confirmation))
...
linelist_new %>%
  filter(state=="WA",date_confirmation >= Sys.Date()%m-% months(2)) %>%
  group_by(state, date_confirmation, test_type) %>%
  summarise(cases = n())%>%
  arrange(desc(date_confirmation)) %>%
  ggplot() +
  geom_bar(
    aes(
      x = date_confirmation,
...
source("R/watermelon_plot_redo.R")
